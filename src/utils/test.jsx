...
const columns = [
  ...
  { id: "image", label: "Зураг", minWidth: 170, align: "right", border: 3 },
...
export default function CustomTable() {
  ...
  useEffect(() => {
    ...
    const formattedData = products.map((product) => (
      ...
      subcategory: subTitles.find((item) => item._id === product.subcategory)?.title || '',
    }));
  ...
  const handleSubmit = async () => {
    // const filteredId = subTitles.find((item) => item._id === formData.subcategory);
          ...
          <Box display="flex" flexDirection="column" gap={3} mt={1}>
            ...
            >
              ...
              {subTitles.map((subtitle) => (
                <option key={subtitle._id} value={subtitle._id || subtitle.id}>
                  {subtitle.title}
                </option>
              ))}
            </select>
